...
ParSens <- function(pars, driftData, n_days, xmx, xmn, ymx, ymn, m, rel_location, rel_months, parallel,
                    ...
                    # This allows ParSens to run a single simulation for each adjusted parameter value and determine the different between those outputs and the output returned by the original parameter set.
                    # Next parSens has two additional arguments that allow users to control the extent of their selected sensitivity tests.
                    s = 0.001, # scale of the deviation in parameter values to be imposed. Corresponds with the percentage of change to be imposed in parameter values. Defaults to 0.01
  ...
  cat('Depending on the number of parameters being tested this may take a few minutes to complete. Hang in there!\n')
  
  # Implement baseline simulation with a call to the JellySim function.
  set.seed(56239) # fix random number seed across repeated simulations to ensure consistent stochasticity across iterations.
  ...
  if(any(params == 'all')){
    ...
    for(ii in 1:length(pars)){
      ...
      # Re-run simulation
      set.seed(56239) # reset random number seed.
