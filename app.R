    ...
    tabPanel(strong("Results"),
             ...
             tabsetPanel(
               ...
               tabPanel("Stochastic variability",
                        h5(p(style = "text-align: center", "Plots displayed on this tab show the level of uncertainty associated with predicted density patterns.")),
...
server <- function(input, output, session) { # inputs are the user define variables & outputs are the visuals returned to the user.
  ...
  sim_outputs <- reactive({
    ...
    # create maps
    Jelly_plots <- SimPlot(meanRast= sim_data()$mean, confRast = sim_data()$conf,
                            sites = sim_data()$site,
